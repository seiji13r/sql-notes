...
-- 3. How can you produce a list of facilities that charge a fee to members?
SELECT name, membercost FROM cd.facilities WHERE membercost > 0 AND membercost IS NOT NULL;

-- 4. How can you produce a list of facilities that charge a fee to members, and that fee is less than 1/50th of the monthly maintenance cost? Return the facid, facility name, member cost, and monthly maintenance of the facilities in question.
SELECT facid, name, membercost, monthlymaintenance, ROUND(monthlymaintenance/50,2)
FROM cd.facilities
WHERE membercost > 0
...
WHERE 
	cd.facilities.name LIKE '%ennis%' AND
	cd.bookings.starttime BETWEEN '2012-09-21' AND '2012-09-22'
ORDER BY
...
SELECT
	cd.bookings.bookid,
	cd.bookings.memid,
	cd.members.firstname || ' ' || cd.members.surname AS fullname,
	cd.bookings.starttime
FROM cd.bookings
...
ORDER BY cd.bookings.starttime DESC;
```